...
class ViewpointInvarianceConfig(GeneratorConfig):
    ...
    azimuth_lim: list = field(
        default_factory=lambda: [0, 365], metadata={"label": "azimuth limits"}
    )
    inclination_lim: list = field(
...
def generate_all(config: ViewpointInvarianceConfig):
    ...
    with open(output_folder / "annotation.csv", "w", newline="") as annfile:
        ...
        for img_path in tqdm(all_images):
            ...
            if not (
                config.inclination_lim[0] <= inclination <= config.inclination_lim[1]
            ):
                continue

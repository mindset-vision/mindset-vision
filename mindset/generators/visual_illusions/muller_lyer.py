...
class DrawMullerLyer(DrawStimuli):
    ...
    def generate_illusion(
        ...
        if type == "scrambled":
            draw_arrow(
                d, get_arrow_rnd_pos(), theta=arrow_angle,
                angle_arrow=arrow_cap_angle, arrow_length=arrow_length,
                color=self.fill, width=self.line_args["width"],
            )
            draw_arrow(
                d, get_arrow_rnd_pos(), theta=arrow_angle + 180,
                angle_arrow=arrow_cap_angle, arrow_length=arrow_length,
                color=self.fill, width=self.line_args["width"],
            )
            d.line(
            ...
            draw_arrow(
                d, (line_position[0] - line_length // 2, line_position[1]),

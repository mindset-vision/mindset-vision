    ...
    with open(output_folder / "annotation.csv", "w", newline="") as annfile:
        ...
        for c in conditions:
            ...
            for i in tqdm(range(n), desc=c):
                img, line_length, line_position, arrow_length, cap_arrows_angle, angle_arrow = ds.generate_illusion(condition=c)
                if config.antialiasing:
                    img = apply_antialiasing(img)
                unique_hex = uuid.uuid4().hex[:8]
                path = Path(c) / f"{unique_hex}.png"
                img.save(output_folder / path)
                writer.writerow([path, c, ds.background, line_length, line_position, arrow_length, cap_arrows_angle, angle_arrow, i])
